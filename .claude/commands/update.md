...
This command upgrades the GreyCat skill by:
1. Installing the latest GreyCat libraries from `project.gcl`
2. Deleting all old GCL files from the skill to ensure a clean state
3. Replacing all GCL files in the skill with the newly installed versions
4. Updating library versions in skill markdown documentation
5. Analyzing function signatures and updating skill documentation
6. Re-packaging the skill
...
**IMPORTANT**: First, delete all existing `.gcl` files from the `./greycat` folder to remove any stale files:

```bash
find ./greycat -type f -name "*.gcl" -delete
```
