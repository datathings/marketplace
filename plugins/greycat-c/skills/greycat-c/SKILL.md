...
- **`TensorDistance` gained `lorentz` and `poincare`** (hyperbolic distances — Lorentz/hyperboloid model and Poincaré ball model, both curvature fixed at -1). **`Identity.set_role(name, role): bool`** is a new admin-only static native (false if `name` not found, throws if `role` is unknown) — same-version additions, no header/ABI changes.
