...
Comprehensive reference for GreyCat native development (C API), the GCL Standard Library, and plugin development patterns. Tracks SDK **8.1** (headers re-verified 2026-07-18: doc-comment-only upstream changes to `alloc.h`/`string.h`/`table.h`; corrected a stale claim that `gc_string_t.buffer` is not NUL-terminated — it always is).
